...
def wiget_enable(param):
    for i in param:
        window[i].Widget.configure(state='normal')
def wiget_disable(param):
    for i in param:
        window[i].Widget.configure(state='disabled')
...
def stmcptretoggle():  #Storm Capture
    if values['-TOG_STM_CPTRE-'] == True: wiget_enable(['-TOG_HRCNE-','-STM_MIN_TEMP-','-STM_MAX_TEMP-','-STM_MIN_WIND-','-STM_MIN_CLCT-','-STM_FIN_CLCT-','-STM_GPI-','-STM_MPI-','-STM_LATM_MAX_WIND-','-STM_VNT_IDX-','-STM_MIN_TMESTPS-','-STM_MAX_TMESTPS-','-STM_VRMI-','-STM_LAV-'])
    else: wiget_disable(['-TOG_HRCNE-','-STM_MIN_TEMP-','-STM_MAX_TEMP-','-STM_MIN_WIND-','-STM_MIN_CLCT-','-STM_FIN_CLCT-','-STM_GPI-','-STM_MPI-','-STM_LATM_MAX_WIND-','-STM_VNT_IDX-','-STM_MIN_TMESTPS-','-STM_MAX_TMESTPS-','-STM_VRMI-','-STM_LAV-'])
def baltoggle ():      #Run To Balance
    if values['-TOG_RUN_TO_BLNCE-'] == True:
        wiget_disable(['-PHY_RUNTME-'])
        wiget_enable(['-PHY_THRSHLD-','-PHY_BSELNE-','-PHY_MAX_YR-','-PHY_MIN_YR-'])
    else:
        wiget_enable(['-PHY_RUNTME-'])
        wiget_disable(['-PHY_THRSHLD-','-PHY_BSELNE-','-PHY_MAX_YR-','-PHY_MIN_YR-'])
def keplertoggle():    #Keplerian Orbit
    if values['-TOG_KPLR_OBT-'] == True: wiget_enable(['-OBT_MNANMLY-'])
    else: wiget_disable(['-OBT_MNANMLY-'])
def tidaltoggle():     #Tidally Locked
    if values['-TOG_TDL_LCK-'] == True: wiget_enable(['-PLNT_SUB_LONG-','-PLNT_SUB_DSNC-','-PLNT_TMP_CNST-'])
    else: wiget_disable(['-PLNT_SUB_LONG-','-PLNT_SUB_DSNC-','-PLNT_TMP_CNST-'])
def pressuretoggle():  #Pressure
    if values['-TOG_BLK_PR-'] == True: wiget_enable(['-BLK_PR-'])
    else: wiget_disable(['-BLK_PR-'])
def gascontoggle():    #Gas Constant
    if values['-TOG_GAS_CNST-'] == True: wiget_enable(['-GAS_CNST-'])
    else: wiget_disable(['-GAS_CNST-'])
def ptoggle():         #Gas Pressure
    if values['-TOG_GAS_PR-'] == True: wiget_enable(['-ATM_H2-','-ATM_HE-','-ATM_N2-','-ATM_O2-','-ATM_AR-','-ATM_NE-','-ATM_KR-','-ATM_H2O-','-ATM_CO2-'])
    else: wiget_disable(['-ATM_H2-','-ATM_HE-','-ATM_N2-','-ATM_O2-','-ATM_AR-','-ATM_NE-','-ATM_KR-','-ATM_H2O-','-ATM_CO2-'])
def aquatoggle():      #Aqua Planet
    if values['-TOG_PLT_AQA-'] == True: wiget_disable(['-TOG_IMG_SRA-','-TOG_PLT_DST-','-HGHTMP_IMG_FLE-','-HGHTMP_IMG_OPN-','-HGHTMP_WTR_TRSHLD-','-HGHTMP_ELE_HGH-','-HGHTMP_ELE_LOW-','-TOG_IMG_DBG-','-HGHTMP_SRA_NME-','-HGHTMP_LAND_SRA-','-LAND_SRA_OPN-','-HGHTMP_TOPO_SRA-','-TOPO_SRA_OPN-'])
    else:
        wiget_enable(['-TOG_IMG_SRA-','-TOG_PLT_DST-'])
        if values['-TOG_IMG_SRA-'] == False:
            wiget_enable(['-HGHTMP_IMG_FLE-','-HGHTMP_IMG_OPN-','-HGHTMP_WTR_TRSHLD-','-HGHTMP_ELE_HGH-','-TOG_IMG_DBG-','-HGHTMP_SRA_NME-'])
            if values['-TOG_PLT_DST-'] == True: wiget_enable(['-HGHTMP_ELE_LOW-'])
        elif values['-TOG_IMG_SRA-'] == True: wiget_enable(['-HGHTMP_LAND_SRA-','-LAND_SRA_OPN-','-HGHTMP_TOPO_SRA-','-TOPO_SRA_OPN-'])
def imgsratoggle():    #Image/SRA
    if values['-TOG_IMG_SRA-'] == True:
        window['-DIS_IMG_SRA-'].update('SRA Mode')
        wiget_disable(['-HGHTMP_IMG_FLE-','-HGHTMP_IMG_OPN-','-HGHTMP_WTR_TRSHLD-','-HGHTMP_ELE_HGH-','-HGHTMP_ELE_LOW-','-TOG_IMG_DBG-','-HGHTMP_SRA_NME-'])
        wiget_enable(['-HGHTMP_LAND_SRA-','-LAND_SRA_OPN-','-LAND_SRA_OPN-','-HGHTMP_TOPO_SRA-','-TOPO_SRA_OPN-'])
    else:
        window['-DIS_IMG_SRA-'].update('Image Mode')
        wiget_enable(['-HGHTMP_IMG_FLE-','-HGHTMP_IMG_OPN-','-HGHTMP_WTR_TRSHLD-','-HGHTMP_ELE_HGH-','-TOG_IMG_DBG-','-HGHTMP_SRA_NME-'])
        wiget_disable(['-HGHTMP_LAND_SRA-','-LAND_SRA_OPN-','-LAND_SRA_OPN-','-HGHTMP_TOPO_SRA-','-TOPO_SRA_OPN-'])
        if values['-TOG_PLT_DST-'] == True: wiget_enable(['-HGHTMP_ELE_LOW-'])
def dsrtoggle():       #Desert Planet
    if values['-TOG_PLT_DST-'] == True:
        wiget_disable(['-TOG_PLT_AQA-'])
        if values['-TOG_IMG_SRA-'] == False: wiget_enable(['-HGHTMP_ELE_LOW-'])
    else:
        wiget_enable(['-TOG_PLT_AQA-'])
        wiget_disable(['-HGHTMP_ELE_LOW-'])
def soilalbtoggle():   #Soil Albedo
    if values['-TOG_SOIL_ALB-'] == True: wiget_enable(['-SOIL_ALB-'])
    else: wiget_disable(['-SOIL_ALB-'])
def soildepthtoggle(): #Soil Depth
    if values['-TOG_SOIL_DPTH-'] == True: wiget_enable(['-SOIL_DPTH-'])
    else: wiget_disable(['-SOIL_DPTH-'])
def capsoiltoggle():   #Soil Heat Capacity
    if values['-TOG_SOIL_HCAP-'] == True: wiget_enable(['-SOIL_HCAP-'])
    else: wiget_disable(['-SOIL_HCAP-'])
def soilwcptoggle():   #Soil Water Capacity
    if values['-TOG_SOIL_WCAP-'] == True: wiget_enable(['-SOIL_WCAP-'])
    else: wiget_disable(['-SOIL_WCAP-'])
def soilsattoggle():   #Soil Saturation
    if values['-TOG_SOIL_SAT-'] == True: wiget_enable(['-SOIL_SAT-'])
    else: wiget_disable(['-SOIL_SAT-'])
def snowalbtoggle():   #Snow Albedo
    if values['-TOG_SNW_ALB-'] == True: wiget_enable(['-SNW_ALB-'])
    else: wiget_disable(['-SNW_ALB-'])
def mxsnowtoggle():    #Max Snow
    if values['-TOG_MAX_SNW-'] == True: wiget_enable(['-MAX_SNW-'])
    else: wiget_disable(['-MAX_SNW-'])
def oceanalbtoggle():  #Ocean Albedo
    if values['-TOG_OCN_ALB-'] == True: wiget_enable(['-OCN_ALB-'])
    else: wiget_disable(['-OCN_ALB-'])
def mldepthtoggle():   #Mixed Layer Depth
    if values['-TOG_OCN_MLD-'] == True: wiget_enable(['-OCN_MLD-'])
    else: wiget_disable(['-OCN_MLD-'])
def gtoggle():         #Glacier
    if values['-TOG_GLCR-'] == True: wiget_enable(['-GLCR_HGT-','-GLCR_TRSHLD-'])
    else: wiget_disable(['-GLCR_HGT-','-GLCR_TRSHLD-'])
def vegtoggle():       #Vegetation
    if values['-TOG_VEG_TYPE-'] != 'None': wiget_enable(['-VEG_ACC-','-VEG_BIOM_GRTH-','-VEG_INIT_GRTH-','-VEG_STOM_C-','-VEG_RGH-','-VEG_SCC-','-VEG_PCC-'])
    else: wiget_disable(['-VEG_ACC-','-VEG_BIOM_GRTH-','-VEG_INIT_GRTH-','-VEG_STOM_C-','-VEG_RGH-','-VEG_SCC-','-VEG_PCC-'])
def arsltoggle():      #Aerosols
    if values['-TOG_ARSLS-'] == True: wiget_enable(['-TOG_BLK_ATM-','-TOG_ARSL_TYPE-','-ARSL_DNSTY-','-ARSL_MS_MX_RTO-','-ARSL_RDUS-'])
    else: wiget_disable(['-TOG_BLK_ATM-','-TOG_ARSL_TYPE-','-ARSL_DNSTY-','-ARSL_MS_MX_RTO-','-ARSL_RDUS-'])
...
def load_ini(filepath):
    ...
    window['-PHY_TMESTPS-'].update(simnprm.get('Timestep'))
    window['-PHY_RUNSTPS-'].update(simnprm.get('Runsteps'))
    window['-PHY_SNPSHTS-'].update(simnprm.get('Snapshots'))
    window['-PHY_NSTPW-'].update(simnprm.get('NSTPW'))
    window['-PHY_RSTRT_FLE-'].update(simnprm.get('Restart File'))
    window['-PHY_FLTR-'].update(simnprm.get('Physics Filter'))
    window['-PHY_FLTR_APP-'].update(simnprm.get('Filter Application'))
    window['-TOG_RUN_TO_BLNCE-'].update(simnprm.get('Run To Balance'))
    window['-PHY_RUNTME-'].update(simnprm.get('Run Time'))
    window['-PHY_THRSHLD-'].update(simnprm.get('Threshold'))
    window['-PHY_BSELNE-'].update(simnprm.get('Baseline'))
    window['-PHY_MIN_YR-'].update(simnprm.get('Min. Year'))
    window['-PHY_MAX_YR-'].update(simnprm.get('Max. Year'))
    window['-TOG_CRSH_IF_BRKN-'].update(simnprm.get('Crash if Broken'))
    window['-TOG_CLN-'].update(simnprm.get('Clean'))
    window['-TOG_ALL_YRS-'].update(simnprm.get('All Years'))
    window['-TOG_KEEP_RSTS-'].update(simnprm.get('Keep Restarts'))
    starprm = cfg['Star']
