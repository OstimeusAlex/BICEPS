HELP_DICT = {
    ...
    "helpkplrobt" :
    """Help: Keplerian Orbit
    False by Default; If True, a generic Keplerian orbital calculation will be performed. This means no orbital precession, Milankovich cycles, etc,
    but does allow for accurate calculation of a wide diversity of orbits, including with higher eccentricity.

    Note that extreme orbits may have extreme results, including extreme crashes.""",

    "helpmnanmly" :
    """Help: Mean Anomaly
    358.617 by Default; The initial mean anomaly in degrees. Only used if keplerian=True.""",
    ...
    "helprsure" :
    """Help: Pressure
    1.0 bar by Default; Total surface pressure in bars.
    ...
    "helpgsprsurs" :
    """Help: Gas Pressures (AKA Partial Pressures) (bar)
    ...
    "helparsls" :
    """Help: Aerosols
    False by default; Compute aerosol transport.""",

    "helpblkatm" :
    """Help: Bulk Atmosphere
    N2 by Default; Type of bulk atmosphere for aerosol suspension.
    If 1, N2 is assumed for the dominant bulk molecule in the atmosphere.
    If 2, H2 is assumed.
    If 3, CO2 is assumed.""",

    "helparsltype" :
    """Help: Aerosol Type
    Photochemical by default; Type of haze source.
    If 1, photochemical haze is produced in the top model layer.
    If 2, the aerosol is dust and is produced from the surface.""",

    "helparsldnsty" :
    """Help: Aerosol Density
    1000.0 kg/m3 by Default; Density of the aerosol particle.""",

    "helparslmmr" :
    """Help: Aerosol Mass Mixing Ratio
    1e-12 kg/kg by Default; Initial haze mass mixing ratio.""",

    "helparslrdus" :
    """Help: Aerosol Particle Radius
    50nm (5e-09) by Default; Aerosol particle radius in meters.""",
    ...
    "helphghcdncestrt" :
    """Help: High Cadence Start
    Timestep at which high-cadence output should begin.""",

    "helphghcdnceend" :
    """Help: High Cadence End
    Timestep at which high-cadence output should end.""",

    "helphghcdnceintrvl" :
    """Help: High Cadence Interval
    How many timesteps should elapse between high-cadence outputs.""",

    "helpstmcptre" :
    """Help: Storm Capture
    False by Default; A dictionary containing arguments controlling when high-cadence output is triggered by storm activity.""",

    "helphrcnegns" :
    """Help: Hurricane Cyclogenesis
    False by Default; Whether or not to use the Komacek, et al 2020 conditions for hurricane cyclogenesis as the output trigger.""",

    "helpminsfcetmp" :
    """Help: Min Surface Temperature
    298.15K by Default; Min. surface temperature for storm activity.""",

    "helpmaxsfcetmp" :
    """Help: Max Surface Temperature
    373.15K by Default; Max. surface temperature for storm activity.""",

    "helpminsfcews" :
    """Help: Min Surface Windspeed
    20.5 m/s by Default; Minimum surface windspeed for storm activity.""",

    "helpmincelcount" :
    """Help: Min Cell Count
    30 by Default; Minimum number of cells that must trigger to start output.""",

    "helpfnlcelcount" :
    """Help: Final Cell Count
    16 by Default; Minimum number of cells at which point storm output ends.""",

    "helpgpithrshld" :
    """Help: Genesis Potential Index
    0.37 by Default; Genesis Potential Index threshold.""",

    "helpmpithrshld" :
    """Help: Max Potential Intensity
    33 m/s by Default; Max potential intensity threshold for nktrigger output.""",

    "helplamwthrshld" :
    """Help: Low Atm. Max Wind
    33 m/s by Default; Lower-atmosphere maximum wind threshold for storm activity.""",

    "helpvntidxthrshld" :
    """Help: Ventilation Index
    0.145 by Default; Ventilation index threshold for nktrigger output.""",

    "helpmintmestps" :
    """Help: Min Timesteps
    256 by Default; Minimum number of timesteps to write output.""",

    "helpmaxtmestps" :
    """Help: Max Timesteps
    1024 by Default; Maximum number of timesteps to write output.""",

    "helpvrmithrshld" :
    """Help: Vent-Reduced Max Intensity
    0.577 by Default; Ventilation-reduced maximum intensity threshold.""",

    "helplmvthrshld" :
    """Help: Low Atm. Vorticity
    1.2*10^-5 s^-1 by Default; Lower-atmosphere vorticity threshold for nktrigger output.""",
...
def helpTooltipText(entry):
    return str(HELP_DICT[entry])
